...
def main(context):
    ...
    ab = [uve[0].x - (uve[0].x + uve[1].x)/2,
          uve[0].y - (uve[0].y + uve[1].y)/2]
    ...
    while(_uv_ == 0 and i != len(uve)):
        ab = [uve[0].x - (uve[0].x + uve[i].x)/2,
              uve[0].y - (uve[0].y + uve[i].y)/2]
    ...
    bpy.ops.transform.rotate(value=math.radians(angle), orient_axis='Z', orient_type='VIEW', orient_matrix=((-1, -0, -0), (-0, -1, -0), (-0, -0, -1)), orient_matrix_type='VIEW',
...
def Pack_together(self, context):
    ...
    if len(UV_verts) == 2:
        ...
        distance = math.sqrt(
            (UV_verts[0].x - UV_verts[1].x)**2 + (UV_verts[0].y - UV_verts[1].y)**2)
        ...
        bpy.ops.transform.resize(value=(Scale, Scale, Scale), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', mirror=True,
...
class UV_OT_AlignUV(bpy.types.Operator):
    """Align Island to closest x or y axis by selected edge"""
    bl_idname = "uv.alignuv"
    bl_label = "Align UV Island"
    ...
    def execute(self, context):
        ...
        if bpy.context.scene.tool_settings.use_uv_select_sync == True:
            self.report(
                {"WARNING"}, "'Align UV Island' works when UV Sync is off")
        else:
            main(context)
...
class UV_OT_Pack(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.context.scene.tool_settings.use_uv_select_sync == True:
            self.report(
                {"WARNING"}, "'Pack together' works when UV Sync is off")
        else:
...
def register():
    ...
    bpy.types.Scene.uv_tool = PointerProperty(type=MySettings)
